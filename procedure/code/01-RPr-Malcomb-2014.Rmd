...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Katie Heo, Middlebury Open GIScience Classes"
date: "`r Sys.Date()`"
...
In our reproduction study, the data visualized in figure 4 of the original study and referred to as 40% Adaptive Capacity in table 2 was referred to as **adaptive capacity** while the data visualized in figure 5 of the original study and referred to as 100% Household Resilience in table 2 was referred to as **vulnerability**.
...
```
The comparison of Figure 4 in the reproduction study and the original study reflects some areas in Northern and Southern Malawi where the reproduction study has a lower adaptive capacity than the original study (resulting in -2). Most areas reflect differences of 0 to -1, implying that our study calculated slightly lower adaptive capacity of Malawi than the original study. 
...
```
The additive vulnerability scores are between 33.08 and 82.94. The map reflects medium to high vulnerability in most areas (ranging around the 50-70 area).

```{r find break & limit values for vulnerability map-1}
...
### Comparison with Geometric Vulnerability score

Calculate an aggregated vulnerability score by multiplying low adaptive capacity (invert adaptive capacity by subtracting from the maximum score of 40), livelihood sensitivity, and drought exposure added to flood risk.

$$
Vulnerability = (40 - Adaptive Capacity) * Livelihood Sensitivity * (Drought Exposure + Flood Risk)
$$
```{r combined-vulnerability-2}
# final output (multiplying component rasters)
vulnerability_r_2 <- (40 - capacity_r) * (sensitivity_r) * (drought_r + flood_r) 
# 100 = highest vulnerability, 0 = lowest vulnerability
names(vulnerability_r_2) <- "vulnerability"
plot(vulnerability_r_2)
```
The geometric vulnerability scores are between 0 and 16,209.26. The high areas of vulnerability that can be identified are in the north and south of Malawi, whereas the low areas of vulnerability that can be identified are in central Malawi.


```{r find break & limit values for vulnerability map-2}
vuln_max <- max(vulnerability_r_2[[1]],na.rm = TRUE)
vuln_min <- min(vulnerability_r_2[[1]],na.rm = TRUE)
```
...
vuln_map_2 = ggplot() +
  geom_stars(data = vulnerability_r_2) +
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(vuln_min, vuln_max),
  ...
  labs(title = "Malawi Vulnerability to Climate Change, Geometric") +
  theme_minimal() +
  theme(
    legend.title = element_blank(),
    axis.title.x = element_blank(),
...
In conclusion, our replication study aimed to reproduce the 2010 household resilience (F4) and climate vulnerability (F5) assessment in Malawi. We clarified definitions of "adaptive capacity" and "vulnerability" in our reproduction study as the original study vaguely outlined the term definitions. The replication process involved various components, including recalculating adaptive capacity scores, rescaling indicators, and exploring different methods for aggregating vulnerability scores. While our study was generally successful in replicating several aspects of the original analysis, we encountered spots in Malawi that reflected different adaptive capacity. We attribute this to limitations in data availability and a lack of clarity regarding the original study's decisions. These differences, particularly in the treatment of geometry errors and missing data, could not be definitely determined from the original study. 

One aspect to think about in our study is the comparison map of Figure 4. The map compared adaptive capacity in our study vs the original study, and most areas reflect differences of 0 to -1, implying that our study calculated slightly lower adaptive capacity of Malawi than the original study. This may be due to methodological differences as there were minor variations to the original study. 

Another important aspect of our study was the comparison of the additive and geometric aggregation methods for vulnerability assessment. The original study used the additive method, which resulted in high vulnerability scores for most areas in Malawi. This made it challenging to identify regions with a vulnerability score of 0. Implementing a geometric aggregation method, on the other hand, allowed for a broader range of vulnerability scores that included 0 and went up to 16,209. Using the geometric aggregation method resulted in more areas with lower vulnerability as seen in Figure 6. The difference was particularly evident when examining the maps in Figure 5 and 6. The additive method identified more regions in Malawi as highly vulnerable, whereas the geometric method predominantly highlighted the most vulnerable areas. Geometric aggregation acknowledges the interactions between indicators and provides a more nuanced perspective on vulnerability (Tate 2012).

Our study emphasizes the importance of clarity and transparency in reporting methodologies and data sources, as discrepancies in data availability and treatment can lead to variations in results. Additionally, the choice of aggregation method for vulnerability assessment is critical, as it can significantly impact the identification of vulnerable areas and influence resource allocation for climate adaptation initiatives. 

However, our study is not without limitations. It is based on a limited population interview sample from Malawi, and it relies on private DHS GPS data that does not permit direct redistribution, making it challenging for others to reproduce our work. Therefore, future research in this field should prioritize access to data, transparent reporting, and the exploration of alternative aggregation methods to enhance the accuracy and utility of climate vulnerability assessments in Malawi and similar regions. These efforts are vital to inform effective climate resilience strategies and foster sustainable development in the face of climate change. 
